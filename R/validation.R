...
error_if_counts_is_na = function(.data, .abundance) {
	...
	if (.data %>% filter(!!.abundance %>% is.na) %>% nrow %>% `>` (0))
		stop("tidygate says: You have NA values in your counts")
...
error_if_log_transformed <- function(x, .abundance) {
	...
	if (x %>% nrow %>% `>` (0))
		if (x %>% summarise(m = !!.abundance %>% max) %>% pull(m) < 50)
			stop(
				"tidygate says: The input was log transformed, this algorithm requires raw (un-scaled) read counts"
			)
}
...
error_if_duplicated_genes <- function(.data,
	...
	if (duplicates %>% nrow() > 0) {
		...
		stop(
			"tidygate says: Your dataset include duplicated element/feature pairs. Please, remove redundancies before proceeding."
		)
	}
...
error_if_wrong_input = function(.data, list_input, expected_type) {
	...
	if (
		...
		stop("tidygate says: You have passed the wrong argument to the function. Please check again.")
...
check_if_duplicated_genes <- function(.data,
	...
	if (!is_unique) {
		message("tidygate says: Those are the duplicated elements/features pair")
		duplicates %>% 
			as.data.frame %>%
			head %>% 
...
eliminate_sparse_transcripts = function(.data, .transcript){
	...
	warning("tidygate says: Some transcripts have been omitted from the analysis because not present in every sample.")
...
check_if_data_rectangular = function(.data, .sample, .transcript, .abundance, type = "hard"){
	...
	# if(!is_rectangular & type == "hard") stop("tidygate says: the data must have the same number of transcript per sample.")
	#
	# if(!is_rectangular & type == "soft") warning("tidygate says: the data should have the same number of transcript per sample.")
...
validation_default = function(.data,
	...
	if(c(
		...
		quo_names(.transcript),
		quo_names(.abundance)
	) %>% duplicated %>% any) stop("tidygate says: among your column names .element, .feature, .value there are some duplictaes.")
	...
	if (type == "hard" &
			!is_present)
		stop(
			"tidygate says: One or more columns .sample .transcript or .abundance are missing from your data frame."
		)
	if (type == "soft" & !is_present) {
		warning(
			"tidygate says: One or more columns .sample .transcript or .abundance are missing from your data frame. The tidygate object has been converted to a `tbl`"
		)
		return(.data %>% tidygate_to_tbl)
	}
	...
	if (type == "hard" &
			!is_type)
		stop(
			"tidygate says: The column provided as .sample .transcript or .abundance do not comply with the required types (<FACTOR/CHARACTER>, <FACTOR/CHARACTER>, <NUMERIC>)."
		)
	if (type == "soft" & !is_type) {
		warning(
			"tidygate says: The column provided as .sample .transcript or .abundance do not comply with the required types. The tidygate object has been converted to a `tbl`"
		)
		return(.data %>% tidygate_to_tbl)
	}
	...
	if (!skip_dupli_check) {
		...
		if (type == "hard" &
				!is_unique)
			stop(
				"tidygate says: Your dataset include duplicated sample/gene pairs. Please, remove redundancies before proceeding (e.g., aggregate_duplicates())."
			)
		if (type == "soft" & !is_unique) {
			warning(
				"tidygate says: Your dataset include duplicated sample/gene pairs. Please, remove redundancies before proceeding (e.g., aggregate_duplicates()). The tidygate object has been converted to a `tbl`"
			)
			return(.data %>% tidygate_to_tbl)
		}
	}
	...
	if (type == "hard" &
			!is_count_good)
		stop("tidygate says: You have NA values in your counts. Please check your data frame.")
	if (type == "soft" & !is_count_good) {
		warning(
			"tidygate says: You have NA values in your counts. The tidygate object has been converted to a `tbl`"
		)
		return(.data %>% tidygate_to_tbl)
	}
...
# validation.tidygate = function(.data,
# 														 .sample = NULL,
# 														 .transcript = NULL,
# 														 .abundance = NULL,
# 														 type = "hard",
...
# 			!is_attr)
# 		stop(
# 			"tidygate says: The object provided has tidygate class but no attribute containing the column names. Insert a valid tidygate object or provide `.sample`, `.transcript`, `.abundance` column names as arguments "
# 		)
# 	if (type == "soft" & !is_attr) {
# 		warning(
# 			"tidygate says: The object provided has tidygate class but no attribute containing the column names. The tidygate object has been converted to a `tbl`"
# 		)
# 		return(.data %>% tidygate_to_tbl)
# 	}
# 
